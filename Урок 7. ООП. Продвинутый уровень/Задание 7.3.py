"""7.3. Реализовать программу работы с органическими клетками, состоящими из ячеек. Необходимо создать класс Клетка.
...
class Cell:
    ...
    def __sub__(self, other: 'Cell'):
        ...
        if result < 0:
            return "Ошибка! разность количества ячеек двух клеток меньше нуля"
        else:
            return result
    ...
    @staticmethod
    def make_order(cell: 'Cell', quantity_in_row: int) -> str:
        items = '*' * cell.quantity
        chunks = [items[idx:idx + quantity_in_row] for idx in range(0, len(items), quantity_in_row)]
        return '\n'.join(chunks)
...
print(f"Неудачный результат вычитания: {Cell2 - Cell1}")

print(f"Результат умножения: {Cell2 * Cell1}")

print(f"Результат деления: {Cell2 / Cell1}")

print("\nОрганизация по рядам первой клетки\n")
print(Cell.make_order(Cell1, 2))

print("\nОрганизация по рядам второй клетки\n")
print(Cell.make_order(Cell2, 3))
